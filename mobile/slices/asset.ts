...
const slice = createSlice({
  ...
  reducers: {
    ...
    clearAssets(state: AssetState) {
      state.assets = getInitialPage<AssetDTO>();
    },
    clearAssetsHierarchy(state: AssetState) {
  ...
  (criteria: SearchCriteria): AppThunk =>
    async (dispatch) => {
      try {
        ...
        dispatch(slice.actions.clearAssets());
...
export const getAssetChildren =
  (id: number, parents: number[], locationId?: number): AppThunk =>
    async (dispatch) => {
      dispatch(slice.actions.setLoadingGet({ loading: true }));
      const assets = await api.get<AssetDTO[]>(
        `${basePath}/children/${id}${locationId !== undefined ? `?locationId=${locationId}` : ''
        }`
      );
      dispatch(
